...
scattering = Scattering2D(J=2, shape=(64, 64))
...
def confusion_diagnostic(model) :
    ...
    for data, target in test_loader: # We won't load the testing dataset all at once:
        ...
        data_scatter = scattering(data)
        data_scatter.to(device)
        
        output     = model(data_scatter) # Forward pass through the model
    ...
    fig.savefig('metric_confusion_SCATTER.png', dpi=80, bbox_inches="tight")
...
def train(optimizer, model, epoch, fig, loss_line, time_0):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):   # Training loop :
        ...
        """
    train_time.append(process_time() - time_0)
    
    
import pandas as pd
import datetime
out_dir = 'metric/'
metric_name = 'scatter_{}.csv'.format(str(datetime.datetime.now()))
...
# =============================================================================            
    ...
    test_loss_csv.append('{:.4f}'.format(test_loss.data))
    test_accuracy.append('{:.0f}'.format(percentage.data))
    ...
    fig.savefig('metric/accuracy_SCATTER.png', dpi=80, bbox_inches="tight")
